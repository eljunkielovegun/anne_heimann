...
const Input = ({ label, type = 'text', error, ...props }) => (
  <div className="flex flex-col gap-2 w-full">
    ...
    <input
      type={type}
      className={`px-4 py-3 rounded-md border bg-background transition-colors
...
const TextArea = ({ label, error, ...props }) => (
  <div className="flex flex-col gap-2 w-full">
    ...
    <textarea
      className={`px-4 py-3 rounded-md border bg-background transition-colors min-h-[150px]
        ${error ? 'border-red-500 focus:border-red-500' : 'border-primary/20 focus:border-primary'}
        focus:outline-none focus:ring-2 focus:ring-primary/20`}
      {...props}
    />
